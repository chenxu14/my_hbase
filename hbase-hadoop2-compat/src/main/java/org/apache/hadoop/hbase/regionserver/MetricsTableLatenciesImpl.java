...
public class MetricsTableLatenciesImpl extends BaseSourceImpl implements MetricsTableLatencies {
  ...
  private final ConcurrentMap<TableName,TableHistograms> histogramsByTable = new ConcurrentHashMap<>();
  ...
  public static class TableHistograms {
    ...
    final MutableFastCounter scanResponse;
    ...
    TableHistograms(DynamicMetricsRegistry registry, TableName tn) {
      ...
      scanResponse = registry.newCounter(qualifyMetricsName(tn, SCAN_RESPONSE), "SCAN_RESPONSE", 0);
    }
  ...
  public TableHistograms getOrCreateTableHistogram(String tableName) {
    ...
    TableHistograms latency = CollectionUtils.computeIfAbsent(histogramsByTable, tn,
        () -> new TableHistograms(getMetricsRegistry(), tn));
    return latency;
  }
