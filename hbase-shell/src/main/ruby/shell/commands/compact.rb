    ...
    class Compact < Command
      def help
        return <<-EOF
          ...
          Compact a column family using "MOB" type within a table and a date
          hbase> compact 't1', 'c1', 'MOB', '20180111'
        EOF
      end

      def command(table_or_region_name, family = nil, type = "NORMAL", date = nil)
          admin.compact(table_or_region_name, family, type, date)
      end
    end
  end
end
