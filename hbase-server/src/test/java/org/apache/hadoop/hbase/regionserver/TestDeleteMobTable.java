...
public class TestDeleteMobTable {
  ...
  public void testDeleteMobTable() throws Exception {
    ...
    try {
      ...
      assertHasOneMobRow(table, tn, hcd.getNameAsString());
  ...
  public void testMobFamilyDelete() throws Exception {
    ...
    try {
      ...
      TEST_UTIL.getHBaseAdmin().deleteColumn(tn, FAMILY);
  ...
  private String assertHasOneMobRow(Table table, TableName tn, String familyName)
    ...
    Path mobPath = MobUtils.getMobFamilyPath(TEST_UTIL.getConfiguration(), tn, familyName);
    Path dataPath = new Path(mobPath, MobUtils.formatDate(new Date()));
    Path filePath = new Path(dataPath, fileName);
    FileSystem fs = TEST_UTIL.getTestFileSystem();
