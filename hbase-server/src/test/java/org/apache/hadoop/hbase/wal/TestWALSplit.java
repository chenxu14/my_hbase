...
import org.apache.hadoop.hbase.wal.HdfsWALSplitter.CorruptedLogFileException;
...
public class TestWALSplit {
    ...
    try {
      ...
      int count = robber.runAs(new PrivilegedExceptionAction<Integer>() {
        ...
        public Integer run() throws Exception {
          ...
          HdfsWALSplitter.split(HBASEDIR, WALDIR, OLDLOGDIR, fs, conf2, wals);
  ...
  public void testSplitPreservesEdits() throws IOException{
    ...
    HdfsWALSplitter.split(HBASEDIR, WALDIR, OLDLOGDIR, fs, conf, wals);
  ...
  public void testIOEOnOutputThread() throws Exception {
    ...
    HdfsWALSplitter logSplitter = new HdfsWALSplitter(wals,
  ...
  private void retryOverHdfsProblem(final FileSystem spiedFs) throws Exception {
    ...
    try {
      HdfsWALSplitter.split(HBASEDIR, WALDIR, OLDLOGDIR, spiedFs, conf, wals);
  ...
  public void testTerminationAskedByReporter() throws IOException, CorruptedLogFileException {
    ...
    try {
      ...
      boolean ret = HdfsWALSplitter.splitLogFile(
  ...
  public void testConcurrentSplitLogAndReplayRecoverEdit() throws IOException {
    ...
    HdfsWALSplitter logSplitter = new HdfsWALSplitter(wals,
      ...
      protected Writer createWriter(Path logfile)
        ...
        NavigableSet<Path> files = HdfsWALSplitter.getSplitEditFilesSorted(fs, regiondir);
  ...
  private Path[] getLogForRegion(Path rootdir, TableName table, String region)
    ...
    Path editsdir = HdfsWALSplitter.getRegionDirRecoveredEditsDir(HRegion.getRegionDir(tdir,
        Bytes.toString(region.getBytes())));
    FileStatus[] files = fs.listStatus(editsdir, new PathFilter() {
      @Override
      public boolean accept(Path p) {
        if (HdfsWALSplitter.isSequenceIdFile(p)) {
          return false;
        }
        return true;
      }
  ...
  private static void appendCompactionEvent(Writer w, HRegionInfo hri, String[] inputs,
    ...
    desc.setTableName(UnsafeByteOperations.unsafeWrap(hri.getTable().toBytes()))
