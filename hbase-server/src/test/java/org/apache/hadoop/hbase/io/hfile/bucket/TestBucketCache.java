...
public class TestBucketCache {
  ...
  public void testRetrieveUnsupportedIOE() throws Exception {
    try {
      ...
      Assert.assertEquals("Don't understand io engine name for cache - prefix with file:, heap or offheap", e.getMessage());
    }
  }
  ...
  public void testRAMCache() {
    ...
    HFileBlock blk1 = new HFileBlock(BlockType.DATA, size, size, -1, alloc.allocate(length),
        HFileBlock.FILL_HEADER, -1, 52, -1, meta, alloc);
    HFileBlock blk2 = new HFileBlock(BlockType.DATA, size, size, -1, alloc.allocate(length),
