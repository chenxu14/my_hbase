...
public class TestHMobStore {
  ...
  private void init(String methodName, Configuration conf, HTableDescriptor htd,
    ...
    region = new HRegion(tableDir, wals.getWAL(info.getEncodedNameAsBytes(), null), fs, conf,
            info, htd, null);
  ...
  private void init(Configuration conf, HColumnDescriptor hcd)
    ...
    byte[] referenceValue = Bytes.toBytes(mobFilePath.getName());
    ...
    seekKey1 = (KeyValue) MobUtils.createMobRefCell(kv1, referenceValue, tableNameTag);
    seekKey2 = (KeyValue) MobUtils.createMobRefCell(kv2, referenceValue, tableNameTag);
    seekKey3 = (KeyValue) MobUtils.createMobRefCell(kv3, referenceValue, tableNameTag);
  }
  ...
  public void testGetFromMemStore() throws IOException {
    ...
    conf.set("hbase.hstore.engine.class", "org.apache.hadoop.hbase.regionserver.StripeStoreEngine");
