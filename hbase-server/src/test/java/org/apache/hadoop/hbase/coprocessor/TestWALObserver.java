...
public class TestWALObserver {
  ...
  private void verifyWritesSeen(final WAL log, final SampleRegionWALObserver cp,
    ...
    long txid = log.appendData(hri, new WALKey(hri.getEncodedNameAsBytes(), hri.getTable(), now,
  ...
  public void testEmptyWALEditAreNotSeen() throws Exception {
    ...
    try {
      ...
      long txid = log.appendData(hri,
          new WALKey(hri.getEncodedNameAsBytes(), hri.getTable(), now, mvcc, scopes), new WALEdit());
      log.sync(txid);
  ...
  public void testWALCoprocessorReplay() throws Exception {
    ...
    wal.appendData(hri, new WALKey(hri.getEncodedNameAsBytes(), tableName, now, mvcc, scopes), edit);
    // sync to fs.
    wal.sync();
  ...
  private void addWALEdits(final TableName tableName, final HRegionInfo hri, final byte[] rowName,
    ...
    for (int j = 0; j < count; j++) {
      ...
      txid = wal.appendData(hri, new WALKey(hri.getEncodedNameAsBytes(), tableName,
          ee.currentTime(), mvcc), edit);
    }
    if (-1 != txid) {
      wal.sync(txid);
    }
