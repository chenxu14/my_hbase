...
public class TestStripeStoreEngine {
  ...
  private static TestStoreEngine createEngine(Configuration conf) throws Exception {
    ...
    HColumnDescriptor mockColDesc = Mockito.mock(HColumnDescriptor.class);
    when(mockColDesc.isMobEnabled()).thenReturn(false);
    when(store.getFamily()).thenReturn(mockColDesc);
    when(store.getColumnFamilyName()).thenReturn("Test");
    when(regionInfo.getRegionNameAsString()).thenReturn("Test");
    when(store.getRegionInfo()).thenReturn(regionInfo);
