...
public class TestHFileBlock {
    ...
    for (Compression.Algorithm algo : COMPRESSION_ALGORITHMS) {
      for (boolean pread : new boolean[] { false, true }) {
        for (DataBlockEncoding encoding : DataBlockEncoding.values()) {
          ...
          final List<ByteBuff> encodedBlocks = new ArrayList<ByteBuff>();
          for (int blockId = 0; blockId < numBlocks; ++blockId) {
            ...
            ByteBuff encodedResultWithHeader = hbw.cloneUncompressedBuffWithHeader();
  ...
  private long writeBlocks(Random rand, Compression.Algorithm compressAlgo,
    ...
    for (int i = 0; i < NUM_TEST_BLOCKS; ++i) {
      ...
      if (cacheOnWrite) {
        ByteBuff buff = hbw.cloneUncompressedBuffWithHeader();
