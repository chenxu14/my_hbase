...
public class TestWALLockup {
  ...
  static class DummyWALActionsListener extends WALActionsListener.Base {
    ...
    @Override
    public void visitLogEntryBeforeWrite(HTableDescriptor info, WALKey logKey, WALEdit logEdit) {
      if (info.getNameAsString().equalsIgnoreCase("sleep")) {
        try {
          Thread.sleep(1000);
        } catch (InterruptedException e) {
          e.printStackTrace();
...
//      if (info.getNameAsString() .equalsIgnoreCase("DamagedWALException")) {
