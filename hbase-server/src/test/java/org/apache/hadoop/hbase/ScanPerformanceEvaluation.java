...
public class ScanPerformanceEvaluation extends AbstractHBaseTool {
  ...
  protected void testHdfsStreaming(Path filename) throws IOException {
    ...
    Stopwatch fileOpenTimer = Stopwatch.createStarted();
    FSDataInputStream in = fs.open(filename);
    fileOpenTimer.stop();

    long totalBytes = 0;
    Stopwatch streamTimer = Stopwatch.createStarted();
    while (true) {
  ...
  public void testScan() throws IOException {
    Stopwatch tableOpenTimer = Stopwatch.createStarted();
    Table table = new HTable(getConf(), TableName.valueOf(tablename));
    tableOpenTimer.stop();

    Scan scan = getScan();
    Stopwatch scanOpenTimer = Stopwatch.createStarted();
  ...
  public void testSnapshotScan() throws IOException {
    ...
    Stopwatch snapshotRestoreTimer = Stopwatch.createStarted();
