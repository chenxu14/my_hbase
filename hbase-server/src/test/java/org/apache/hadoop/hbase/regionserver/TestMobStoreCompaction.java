...
public class TestMobStoreCompaction {
  ...
  public void testMobCompactionWithBulkload() throws Exception {
    ...
    // > 300(threshold)
    boolean result = region.bulkLoadHFiles(hfiles, true, false, null);
  ...
  private int countMobFiles() throws IOException {
    ...
    if (fs.exists(mobDirPath)) {
      ...
      for(FileStatus file : files){
        res += UTIL.getTestFileSystem().listStatus(file.getPath()).length;
      }
    }
    return res;
  }
  ...
  private long countMobCellsInMetadata() throws IOException {
    ...
    if (fs.exists(mobDirPath)) {
      FileStatus[] files = UTIL.getTestFileSystem().listStatus(mobDirPath);
      for (FileStatus file : files) {
        for(FileStatus f : UTIL.getTestFileSystem().listStatus(file.getPath())){
    ...
    do {
      ...
      for (Cell kv : kvs) {
        ...
        for (FileStatus file : fs.listStatus(familyPath)){
          if(file.isDirectory()){
            assertTrue(fs.exists(new Path(file.getPath(), fileName)));
          }
        }
      }
    } while (hasMore);
  ...
  private int countMobCellsInMobFiles(int expectedNumDelfiles) throws IOException {
    ...
    if (fs.exists(mobDirPath)) {
      for (FileStatus f : fs.listStatus(mobDirPath)) {
        if(MobConstants.MOB_DELFILE_DIR_NAME.equals(f.getPath().getName())){
          numDelfiles += fs.listStatus(f.getPath()).length;
        }
      ...
      List scanners = StoreFileScanner.getScannersForStoreFiles(sfs, false, true, HConstants.LATEST_TIMESTAMP);
      Scan scan = new Scan();
      scan.setMaxVersions(hcd.getMaxVersions());
      long timeToPurgeDeletes = Math.max(conf.getLong("hbase.hstore.time.to.purge.deletes", 0), 0);
      long ttl = HStore.determineTTLFromFamily(hcd);
      ScanInfo scanInfo = new ScanInfo(copyOfConf, hcd, ttl, timeToPurgeDeletes, KeyValue.COMPARATOR);
