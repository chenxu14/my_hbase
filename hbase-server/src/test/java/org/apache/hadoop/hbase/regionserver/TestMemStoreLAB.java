...
public class TestMemStoreLAB {
  ...
  public void testLABThreading() throws Exception {
    ...
    Map<Integer, Map<Integer, AllocRecord>> mapsByChunk =
      Maps.newHashMap();
    int sizeCounted = 0;
    for (AllocRecord rec : Iterables.concat(allocations)) {
      sizeCounted += rec.size;
      if (rec.size == 0) continue;
      Map<Integer, AllocRecord> mapForThisByteArray = mapsByChunk.get(System.identityHashCode(rec.alloc));
      if (mapForThisByteArray == null) {
        mapForThisByteArray = Maps.newTreeMap();
        mapsByChunk.put(System.identityHashCode(rec.alloc), mapForThisByteArray);
      }
