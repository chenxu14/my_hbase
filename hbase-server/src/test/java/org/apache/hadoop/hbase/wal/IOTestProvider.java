...
public class IOTestProvider implements WALProvider {
  ...
  private static class IOTestWAL extends FSHLog {
    ...
    @Override
    protected Writer createWriterInstance(final Path newPath, final Path oldPath) throws IOException {
      ...
      if (!initialized || doFileRolls) {
        ...
        writer.init(fs, newPath, null, conf, false);
        if (!initialized) {
  ...
  private static class IOTestWriter extends ProtobufLogWriter {
    ...
    @Override
    public void init(FileSystem fs, Path path, Path oldPath, Configuration conf, boolean overwritable) throws IOException {
      Collection<String> operations = conf.getStringCollection(ALLOWED_OPERATIONS);
      ...
      super.init(fs, path, oldPath, conf, overwritable);
    }
