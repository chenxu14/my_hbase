...
public class TestWALFactory {
  ...
  public void testSplit() throws IOException {
    ...
    wals.shutdown();
    List<Path> splits = HdfsWALSplitter.split(hbaseDir, logdir, oldLogDir, fs, conf, wals);
  ...
  public void Broken_testSync() throws Exception {
    ...
    try {
      ...
      for (int i = 0; i < total; i++) {
        ...
        wal.appendData(info, new WALKey(info.getEncodedNameAsBytes(), tableName,
  ...
  public void testAppendClose() throws Exception {
    ...
    for (int i = 0; i < total; i++) {
      WALEdit kvs = new WALEdit();
      kvs.add(new KeyValue(Bytes.toBytes(i), tableName.getName(), tableName.getName()));
      wal.appendData(regioninfo, new WALKey(regioninfo.getEncodedNameAsBytes(), tableName,
          System.currentTimeMillis(), mvcc, scopes), kvs);
    }
  ...
  public void testAppend() throws IOException {
    ...
    try {
      ...
      final long txid = log.appendData(hri,
  ...
  public void testVisitors() throws Exception {
    ...
    for (int i = 0; i < COL_COUNT; i++) {
      ...
      cols.add(new KeyValue(row, Bytes.toBytes("column"),
          Bytes.toBytes(Integer.toString(i)),
          timestamp, new byte[]{(byte) (i + '0')}));
      log.appendData(hri, new WALKey(hri.getEncodedNameAsBytes(), tableName,
  ...
  public void testReadLegacyLog() throws IOException {
    ...
    try {
      ...
      sflw.init(fs, path, null, conf, false);
