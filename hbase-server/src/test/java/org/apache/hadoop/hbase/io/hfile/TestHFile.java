...
public class TestHFile extends HBaseTestCase {
  ...
  public static HFileReader createReaderFromStream(ReaderContext context, CacheConfig cacheConf,
      Configuration conf) throws IOException {
    HFileInfo fileInfo = new HFileInfo(context, conf);
    HFileReader preadReader = HFile.createReader(context, fileInfo, cacheConf, conf);
    ...
    context = new ReaderContextBuilder()
        ...
        .build();
    HFileReader streamReader = HFile.createReader(context, fileInfo, cacheConf,  conf);
    return streamReader;
  }
  ...
  public void testReaderWithLRUBlockCache() throws Exception {
    ...
    HFileReader reader = HFile.createReader(fs, storeFilePath, cacheConfig, true, conf);
    long offset = 0;
    while (offset < reader.getTrailer().getLoadOnOpenDataOffset()) {
      ...
      Assert.assertEquals(bufCount, alloc.getFreeBufferCount() + 4); // load-on-open section has 4 block
  ...
  private void readStoreFile(Path storeFilePath, Configuration conf, ByteBuffAllocator alloc)
    ...
    HFileReader reader = HFile.createReader(fs, storeFilePath, cache, true, conf);
    long offset = 0;
    while (offset < reader.getTrailer().getLoadOnOpenDataOffset()) {
  ...
  public void testEmptyHFile() throws IOException {
    ...
    w.close();
    HFileReader r = HFile.createReader(fs, f, cacheConf, true, conf);
  ...
  public void testCorruptTruncatedHFile() throws IOException {
    ...
    HFileWriter w = HFile.getWriterFactory(conf, cacheConf).withPath(this.fs, f)
  ...
  // write them twice
  private int writeSomeRecords(HFileWriter writer, int start, int n, boolean useTags)
      throws IOException {
    String value = "value";
    KeyValue kv;
  ...
  private void writeRecords(HFileWriter writer, boolean useTags) throws IOException {
  ...
  void basicWithSomeCodec(String codec, boolean useTags) throws IOException {
    ...
    HFileContext meta = new HFileContextBuilder()
                        ...
                        .build();
    HFileWriter writer = HFile.getWriterFactory(conf, cacheConf)
    ...
    ReaderContext context = new ReaderContextBuilder().withFileSystemAndPath(fs, ncTFile).build();
    HFileReader reader = createReaderFromStream(context, cacheConf, conf);
  ...
  private void writeNumMetablocks(HFileWriter writer, int n) {
    for (int i = 0; i < n; i++) {
  ...
  private void someTestingWithMetaBlock(HFileWriter writer) {
    writeNumMetablocks(writer, 10);
  }

  private void readNumMetablocks(HFileReader reader, int n) throws IOException {
    for (int i = 0; i < n; i++) {
  ...
  private void someReadingWithMetaBlock(HFileReader reader) throws IOException {
  ...
  public void testNullMetaBlocks() throws Exception {
    ...
    for (Compression.Algorithm compressAlgo : 
      ...
      writer.close();
      fout.close();
      HFileReader reader = HFile.createReader(fs, mFile, cacheConf, true, conf);
