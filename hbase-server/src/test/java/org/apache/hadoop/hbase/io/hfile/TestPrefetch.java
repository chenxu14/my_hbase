...
public class TestPrefetch {
  ...
  @Test(timeout=60000)
  public void testPrefetchHFileMaxSize() throws Exception {
    conf.setLong(CacheConfig.PREFETCH_HFILE_MAXSIZE, -1);
    cacheConf = new CacheConfig(conf);
    Path storeFile = writeStoreFile();
    readStoreFile(storeFile, false);
  }
  ...
  public void testPrefetch() throws Exception {
    cacheConf = new CacheConfig(conf);
    Path storeFile = writeStoreFile();
    readStoreFile(storeFile, true);
  }

  private void readStoreFile(Path storeFilePath, boolean prefetch) throws Exception {
    // Open the file
    ReaderContext context = new ReaderContextBuilder().withFileSystemAndPath(fs, storeFilePath)
        .withSkipPrefetch(false).build();
    HFileInfo hfile = new HFileInfo(context, conf);
    HFileReader reader = HFile.createReader(context, hfile, cacheConf, conf);
    ...
    while (offset < reader.getTrailer().getLoadOnOpenDataOffset()) {
      HFileBlock block = reader.readBlock(offset, onDiskSizeOfNextBlock, false, true, false, true, null, null);
