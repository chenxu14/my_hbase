...
public class TestWALRecordReader {
  ...
  public void testPartialRead() throws Exception {
    ...
    log.appendData(info, getWalKey(ts, scopes), edit);
    edit = new WALEdit();
    edit.add(new KeyValue(rowName, family, Bytes.toBytes("2"), ts+1, value));
    log.appendData(info, getWalKey(ts+1, scopes), edit);
    log.sync();
    ...
    log.appendData(info, getWalKey(ts1+1, scopes), edit);
    edit = new WALEdit();
    edit.add(new KeyValue(rowName, family, Bytes.toBytes("4"), ts1+2, value));
    log.appendData(info, getWalKey(ts1+2, scopes), edit);
    log.sync();
    log.shutdown();
    walfactory.shutdown();
  ...
  public void testWALRecordReader() throws Exception {
    ...
    long txid = log.appendData(info, getWalKey(System.currentTimeMillis(), scopes), edit);
    log.sync(txid);
    ...
    txid = log.appendData(info, getWalKey(System.currentTimeMillis(), scopes), edit);
    log.sync(txid);
    log.shutdown();
    walfactory.shutdown();
