...
public class TestReplicationSourceManager {
  ...
  public void testClaimQueues() throws Exception {
    ...
    ReplicationQueues rq =
        ReplicationFactory.getReplicationQueues(server.getZooKeeper(), server.getConfiguration(),
          server);
    rq.init(server.getServerName().toString(), null);
  ...
  public void testCleanupFailoverQueues() throws Exception {
    ...
    rq1.init(s1.getServerName().toString(), null);
  ...
  public void testNodeFailoverDeadServerParsing() throws Exception {
    ...
    repQueues.init(server.getServerName().toString(), null);
    // populate some znodes in the peer znode
    files.add("log1");
    files.add("log2");
    for (String file : files) {
    ...
    rq2.init(s2.getServerName().toString(), null);
    ...
    rq3.init(s3.getServerName().toString(), null);
  ...
  public void testFailoverDeadServerCversionChange() throws Exception {
    ...
    repQueues.init(s0.getServerName().toString(), null);
    // populate some znodes in the peer znode
    files.add("log1");
    files.add("log2");
    for (String file : files) {
  ...
  static class DummyNodeFailoverWorker extends Thread {
    ...
    public DummyNodeFailoverWorker(String znode, Server s) throws Exception {
      ...
      this.rq =
          ReplicationFactory.getReplicationQueues(server.getZooKeeper(), server.getConfiguration(),
            server);
      this.rq.init(this.server.getServerName().toString(), null);
    }
