...
//    TEST_UTIL.getConfiguration().set("hbase.hstore.engine.class",
//        "org.apache.hadoop.hbase.regionserver.StripeStoreEngine");
  ...
  private Path getFlushedMobFile(Configuration conf, FileSystem fs, TableName table, String family)
    ...
    for (FileStatus hfs : hfFss) {
      if (!hfs.isDirectory()) {
        ...
        return fs.listStatus(hfs.getPath())[0].getPath();
      }
    }
    return null;
  }
  ...
  private void testGetFromArchive(boolean reversed) throws Exception {
    ...
    setUp(defaultThreshold, tn);
    long ts1 = System.currentTimeMillis();
    String dateDir = MobUtils.formatDate(new Date(ts1));
    long ts2 = ts1 + 1;
    long ts3 = ts1 + 2;
    byte [] value = generateMobValue((int)defaultThreshold+1);;
    // Put some data
    ...
    Path targetDir = new Path(mobFamilyPath, dateDir);
    FileStatus[] files = fs.listStatus(targetDir);
    ...
    for(FileStatus file : files) {
      ...
      Path src = new Path(targetDir, filePath.getName());
      Path dst = new Path(storeArchiveDir, filePath.getName());
      fs.rename(src, dst);
    }

    // Verify the moving success
    FileStatus[] files1 = fs.listStatus(targetDir);
  ...
  private static void assertIsMobReference(Cell cell, byte[] row, byte[] family,
    ...
    Path parentPath = new Path(mobFamilyPath, date);
    FileSystem fs = FileSystem.get(TEST_UTIL.getConfiguration());
    Assert.assertTrue(fs.exists(parentPath));
    Path targetPath = new Path(parentPath, fileName);
