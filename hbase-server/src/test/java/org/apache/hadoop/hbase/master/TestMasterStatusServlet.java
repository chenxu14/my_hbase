...
public class TestMasterStatusServlet {
  private static final Log LOG = LogFactory.getLog(TestMasterStatusServlet.class.getName());
  ...
  public void setupBasicMocks() {
    ...
    LoadBalancer balancer = Mockito.mock(SimpleLoadBalancer.class);
    ...
    Mockito.doReturn(balancer).when(am).getBalancer();
    ...
    try {
      Mockito.doReturn(admin).when(clusterConnection).getAdmin();
    } catch (IOException e) {
      LOG.error(e.getMessage(), e);
    }
    Mockito.doReturn(clusterConnection).when(master).getConnection();
  }
