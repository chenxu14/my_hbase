  ...
  public void testListLabels() throws Throwable {
    PrivilegedExceptionAction<ListLabelsResponse> action =
      ...
      public ListLabelsResponse run() throws Exception {
        ...
        assertTrue(labels.contains(ByteStringer.wrap(SECRET.getBytes())));
        assertTrue(labels.contains(ByteStringer.wrap(TOPSECRET.getBytes())));
        assertTrue(labels.contains(ByteStringer.wrap(CONFIDENTIAL.getBytes())));
        assertTrue(labels.contains(ByteStringer.wrap("ABC".getBytes())));
        assertTrue(labels.contains(ByteStringer.wrap("XYZ".getBytes())));
        assertFalse(labels.contains(ByteStringer.wrap(SYSTEM_LABEL.getBytes())));
        return null;
      }
    };
    SUPERUSER.runAs(action);
