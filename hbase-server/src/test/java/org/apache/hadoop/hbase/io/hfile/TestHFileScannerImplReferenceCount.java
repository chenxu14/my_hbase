...
public class TestHFileScannerImplReferenceCount {
  ...
  private void writeHFile(Configuration conf, FileSystem fs, Path hfilePath, Algorithm compression,
    ...
    try (HFileWriter writer =
        new HFileWriterFactory(conf, new CacheConfig(conf)).withPath(fs, hfilePath)
            .withFileContext(context).withComparator(CellComparator.COMPARATOR).create()) {
  ...
  private void testReleaseBlock(Algorithm compression, DataBlockEncoding encoding)
    ...
    BlockCache defaultBC = CacheConfig.instantiateBlockCache(conf);
    ...
    HFileReader reader = HFile.createReader(fs, hfilePath, cacheConfig, true, conf);
    ...
    scanner = (HFileScannerImpl) reader.getScanner(true, true, false);
    int count = 0;
  ...
  public void testWithLruBlockCache() throws Exception {
    ...
    // BlockCache defaultBC = CacheConfig.instantiateBlockCache(conf);
    BlockCache defaultBC = new LruBlockCache(100000, HConstants.DEFAULT_BLOCKSIZE, true, conf);
    CacheConfig cacheConfig = new CacheConfig(conf, null, defaultBC, allocator);
    Assert.assertNotNull(defaultBC);
