...
public class TestHRegionReplayEvents {
    ...
    while (true) {
      ...
      if (flushDesc != null) {
        ...
        if (flushDesc.getAction() == FlushAction.START_FLUSH) {
          ...
          MemstoreSize newMss = store.getSizeToFlush();
  ...
  public void testSecondaryRegionDoesNotWriteRegionEventsToWAL() throws IOException {
    ...
    verify(walSecondary, times(0)).appendData((HRegionInfo)any(),
      (WALKey)any(), (WALEdit)any());
  ...
  public void testReplayBulkLoadEvent() throws IOException {
    ...
    primaryRegion.bulkLoadHFiles(familyPaths, false, false, null);
  ...
  private String createHFileForFamilies(Path testPath, byte[] family,
    ...
    HFileWriterFactory hFileFactory = HFile.getWriterFactoryNoCache(TEST_UTIL.getConfiguration());
