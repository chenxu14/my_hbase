...
public class PartitionedMobCompactor extends MobCompactor {
  ...
  public PartitionedMobCompactor(Configuration conf, FileSystem fs, TableName tableName,
    ...
    Path tempParent = new Path(MobUtils.getMobHome(conf), MobConstants.TEMP_DIR_NAME);
    tempPath = new Path(tempParent, tableName.getNameAsString().replaceAll(":", "_"));
  ...
  protected PartitionedMobCompactionRequest select(List<FileStatus> candidates,
    ...
    if(allFiles){ // only major compaction consider delfile
  ...
  protected List<Path> compactMobFiles(final PartitionedMobCompactionRequest request)
    ...
    try {
      ...
      for (final CompactionPartition partition : partitions) {
        ...
        final List<StoreFile> delFiles = getListOfDelFilesForPartition(partition,
        ...
        results.put(partition.getPartitionId(), pool.submit(new Callable<List<Path>>() {
          ...
          public List<Path> call() throws Exception {
            ...
            return compactMobFilePartition(request, partition, delFiles, conn, table);
          }
        }));
      }
  ...
  private void compactMobFilesInBatch(PartitionedMobCompactionRequest request,
    ...
    try {
      try {
        ...
        do {
          ...
          for (Cell cell : cells) {
            ...
            Cell reference = MobUtils.createMobRefCell(cell, fileName, this.tableNameTag);
      ...
      if (mobCells > 0) {
        // commit mob file
        Path targetPath = new Path(mobFamilyDir, partition.getPartitionId().getLatestDate());
        MobUtils.commitFile(conf, fs, filePath, targetPath, compactionCacheConfig);
        ...
        newFiles.add(new Path(targetPath, filePath.getName()));
      }
      ...
      if (cleanupCommittedMobFile) {
        Path targetPath = new Path(mobFamilyDir, partition.getPartitionId().getLatestDate());
        deletePath(new Path(targetPath, filePath.getName()));
      }
    }
  }
   ...
   * @throws IOException if IO failure is encountered
   */
  private void closeMobFileWriter(StoreFile.Writer writer, long maxSeqId, long mobCellsCount)
    throws IOException {
    if (writer != null) {
   ...
   * @throws IOException if IO failure is encountered
   */
  private void closeRefFileWriter(StoreFile.Writer writer, long maxSeqId, long bulkloadTime)
    throws IOException {
    if (writer != null) {
