...
public class TableAuthManager implements Closeable {
  ...
  private boolean authorizeGlobalUser(TableName table, byte[] family,
      byte[] qualifier, Permission.Action action) {
    if (table == null) table = AccessControlLists.ACL_TABLE_NAME;
    return authorize(getTablePermissions(table).getUser(AccessController.GLOBAL_USER),
  ...
  public boolean authorize(User user, TableName table, byte[] family,
      byte[] qualifier, Permission.Action action) {
    if (authorizeGlobalUser(table, family, qualifier, action)) {
      return true;
    }
