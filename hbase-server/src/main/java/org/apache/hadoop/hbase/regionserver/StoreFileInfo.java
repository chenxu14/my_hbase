...
public class StoreFileInfo {
  ...
  private boolean pefetchNoNeed = false;
   ...
   */
  public StoreFileInfo(final Configuration conf, final FileSystem fs,
  ...
  public StoreFileInfo(final Configuration conf, final FileSystem fs, final FileStatus fileStatus)
      throws IOException {
    this(conf, fs, fileStatus.getPath(), true);
  }
  ...
  StoreFile.Reader createReader(ReaderContext context, CacheConfig cacheConf) throws IOException {
    StoreFile.Reader reader = null;
    if (this.reference != null) {
      reader = new HalfStoreFileReader(context, hfileInfo, cacheConf, reference, refCount, conf);
    } else {
      reader = new StoreFile.Reader(context, hfileInfo, cacheConf, refCount, conf);
    }
    return reader;
  }
  ...
  ReaderContext createReaderContext(boolean doDropBehind, long readahead, ReaderType type)
    ...
    ReaderContextBuilder contextBuilder = new ReaderContextBuilder()
        ...
        .withReaderType(type)
        .withSkipPrefetch(pefetchNoNeed)
        .withFileSystem(fs);
    if (this.reference != null) {
  ...
  public void setPefetchNoNeed(boolean pefetchNoNeed) {
    this.pefetchNoNeed = pefetchNoNeed;
  }
  ...
  Reader preStoreFileReaderOpen(ReaderContext context, CacheConfig cacheConf)
      throws IOException {
    Reader reader = null;
  ...
  Reader postStoreFileReaderOpen(ReaderContext context, CacheConfig cacheConf,
