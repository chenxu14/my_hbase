...
public class AccessControlLists {
  ...
  static void addUserPermission(Configuration conf, UserPermission userPerm)
    ...
    String user = Bytes.toString(userPerm.getUser());
    if ((actions == null) || (actions.length == 0)) {
      String msg = "No actions associated with user '" + user + "'";
      LOG.warn(msg);
      throw new IOException(msg);
    }
    if (AccessController.GLOBAL_USER.equals(user) && userPerm.getTableName() == null) {
      String msg = "Grant perm to GlobalUser should specify TableName!";
      LOG.warn(msg);
      throw new IOException(msg);
    }
