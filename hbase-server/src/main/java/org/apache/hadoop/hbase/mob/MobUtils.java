...
public final class MobUtils {
  ...
  public static void cleanExpiredMobFiles(FileSystem fs, Configuration conf, TableName tableName,
    ...
    LOG.info("MOB HFiles older than " + expireDate.toLocaleString() + " will be deleted!");
    ...
    for (FileStatus file : stats) {
      ...
      try {
        if(file.isDirectory() && !MobConstants.MOB_DELFILE_DIR_NAME.equals(fileName)) { // is a date directory
          Date fileDate = parseDate(fileName);
          if (fileDate.getTime() < expireDate.getTime()) {
            if (LOG.isDebugEnabled()) {
              LOG.debug(fileName + " is an expired folder, remove the mob files!");
            }
            FileStatus[] statsPerDay = fs.listStatus(file.getPath());
            if (statsPerDay.length == 0) {
              fs.delete(file.getPath(), true);
            } else {
              for (FileStatus child : statsPerDay) {
                filesToClean.add(new StoreFile(fs, child.getPath(), conf, cacheConfig, BloomType.NONE));
              }
            }
          }
        } else { // is a file
          ...
          if (fileDate.getTime() < expireDate.getTime()) {
            if (LOG.isDebugEnabled()) {
              LOG.debug(fileName + " is an expired file, remove it!");
            }
  ...
  public static void doMobCompaction(Configuration conf, FileSystem fs, TableName tableName,
    HColumnDescriptor hcd, ExecutorService pool, boolean allFiles, String date, TableLockManager tableLockManager)
      throws IOException {
    ...
    try {
      ...
      if(date != null && !"".equals(date.trim())) {
        compactor.compact(allFiles, date);
      } else {
  ...
  public static boolean isReadEmptyValueOnMobCellMiss(Scan scan) {
    ...
    if(readEmptyValueOnMobCellMiss == null) {
      return true;
    }
    try {
      return Bytes.toBoolean(readEmptyValueOnMobCellMiss);
    } catch (IllegalArgumentException e) {
      return true;
    }
  }
  ...
  public static Cell createMobRefDeleteMarker(Cell cell) {
    return new TagRewriteCell(cell, Tag.concatTags(REF_DELETE_MARKER_TAG_BYTES, cell));
  }
  ...
  public static boolean isMobFileExpired(HColumnDescriptor column, long current, String fileDate) {
    ...
    try {
      ...
      if (date.getTime() + 1000 < expireDate.getTime()) {
        return true;
      }
