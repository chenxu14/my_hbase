...
public class HMobStore extends HStore {
  ...
  public Cell resolve(Cell reference, boolean cacheBlocks, long readPt,
    ...
    if (MobUtils.hasValidMobRefCellValue(reference)) {
      ...
      if (tableNameTag != null) {
        String tableNameString = Bytes.toString(tableNameTag.getValue());
        ...
        result = readCell(new Path(familyPath, date), fileName, reference, cacheBlocks, readPt);
        if(result == null){
          result = readCell(familyPath, fileName, reference, cacheBlocks, readPt);
        }
        if(result == null){
          ...
          result = readCell(archPath, fileName, reference, cacheBlocks, readPt);
        }
      }
    }
    if (result != null) {
   ...
   * @throws IOException
   */
  private Cell readCell(Path location, String fileName, Cell search, boolean cacheMobBlocks, long readPt) {
