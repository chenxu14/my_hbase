...
import org.apache.hadoop.hbase.replication.ReplicationUtil;
...
public class ReplicationSourceManager implements ReplicationListener {
  ...
  private Map<String, Path> latestPaths;
  ...
  public ReplicationSourceManager(final ReplicationQueues replicationQueues,
      ...
      final Configuration conf, final Server server, String group, final FileSystem fs, final Path logDir,
      final Path oldLogDir, final UUID clusterId, WALFileLengthProvider walFileLengthProvider) {
    ...
    synchronized (this.walsById) {
      ...
      synchronized (latestPaths) {
        if (this.latestPaths.size() > 0) {
          LOG.info("latestPaths size is " + latestPaths.size());
          for (Map.Entry<String, Path> log : latestPaths.entrySet()) {
            String walPrefix = log.getKey();
            Path logPath = log.getValue();
  ...
  void preLogRoll(Path newLog) throws IOException {
    ...
    synchronized (latestPaths) {
      ...
      if(LOG.isDebugEnabled()) {
        LOG.debug("latestPaths size is : " + latestPaths.size());
        for(Map.Entry<String, Path> log : latestPaths.entrySet()) {
          LOG.debug("logPrefix:" + log.getKey() + ", logName: " + log.getValue().getName());
        }
      }
    }
  }
  ...
  private void transferQueues(String rsZnode) {
    String groupOfDeadRS = ReplicationUtil.getGroupNameOfServer(server.getZooKeeper(), ServerName.valueOf(rsZnode));
    if (this.group != null && groupOfDeadRS != null && !this.group.equals(groupOfDeadRS)) {
      StringBuilder sb = new StringBuilder("target group of dead RS ");
      sb.append(rsZnode).append(" is ").append(groupOfDeadRS).append(", bypass transfer because our group is " + group);
  ...
  public void peerListChanged(List<String> peerIds) {
    for (String id : peerIds) {
      if (ReplicationUtil.targetPeerNotBelongTo(id, group)) {
        // target peer don not belong to this group, bypass it!
        LOG.info("peerId " + id + " don't belongs to our group[" + group + "], bypass it!");
        continue;
      }
      try {
