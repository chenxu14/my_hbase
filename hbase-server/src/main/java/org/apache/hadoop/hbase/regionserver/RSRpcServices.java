...
public class RSRpcServices implements HBaseRPCErrorHandler,
  ...
  private long maxRowsPerMultiGet;
  ...
  private boolean parallelGetEnable;
  ...
  private void addResults(final ScanResponse.Builder builder, final List<Result> results,
      final PayloadCarryingRpcController controller, boolean isDefaultRegion, boolean clientCellBlockSupported) {
  ...
  private List<CellScannable> doNonAtomicRegionMutation(final Region region,
    ...
    for (ClientProtos.Action action : actions.getActionList()) {
      ...
      try {
        Result r = null;
        if (context != null && context.isAboveQuota(maxQuotaResultSize, maxRowsPerMultiGet)) {
  ...
  private void doParallelGet(List<GetContext> getCtxs, List<CellScannable> cellsToReturn,
    ...
    for (GetContext getCtx : getCtxs) {
      ...
      this.regionServer.service.submit(handler);
  ...
  public RSRpcServices(HRegionServer rs) throws IOException {
    ...
    maxRowsPerMultiGet = rs.conf.getLong(HConstants.RS_MAXROWS_PER_MULTIGET,
      HConstants.DEFAULT_MAXROWS_PER_MULTIGET);
    rpcTimeout = rs.conf.getInt(
      HConstants.HBASE_RPC_TIMEOUT_KEY,
      HConstants.DEFAULT_HBASE_RPC_TIMEOUT);
    parallelGetEnable = rs.conf.getBoolean(HConstants.RS_PARALLEL_GET_ENABLED, false);
  ...
  public void onConfigurationChange(Configuration newConf) {
    ...
    if (priority instanceof AnnotationReadingPriorityFunction) {
      ((AnnotationReadingPriorityFunction)priority).setConf(newConf);
    }
    parallelGetEnable = newConf.getBoolean(HConstants.RS_PARALLEL_GET_ENABLED, false);
    maxRowsPerMultiGet = newConf.getLong(HConstants.RS_MAXROWS_PER_MULTIGET,
        HConstants.DEFAULT_MAXROWS_PER_MULTIGET);
  }
    ...
    try {
      ...
      if (existence != null) {
        ...
        if (isClientCellBlockSupport(call) && controller instanceof PayloadCarryingRpcController
            && VersionInfoUtil.hasMinimumVersion(call.getClientVersionInfo(), 1, 3)) { // HBASE-13158
  ...
  private void scan(PayloadCarryingRpcController controller, ScanRequest request, RegionScannerHolder rsh,
  ...
  public ScanResponse scan(final RpcController controller, final ScanRequest request)
    ...
    try {
      ...
      if (rows > 0) {
        ...
        if (!done) {
          scan((PayloadCarryingRpcController) controller, request, rsh, maxQuotaResultSize, rows, limitOfRows,
