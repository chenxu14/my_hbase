...
public abstract class FlushLargeStoresPolicy extends FlushPolicy {
  ...
  protected boolean shouldFlush(Store store) {
    if (store.getSizeOfMemStore().getHeapSize()
        + store.getSizeOfMemStore().getOffHeapSize() > this.flushSizeLowerBound) {
      if (LOG.isDebugEnabled()) {
        LOG.debug("Flush "+ store.getColumnFamilyName() +" of "+region.getRegionInfo().getEncodedName() +"; "
              + "heap memstoreSize=" + store.getSizeOfMemStore().getHeapSize()
              + "off heap memstoreSize="+ store.getSizeOfMemStore().getOffHeapSize() + " > memstore lowerBound="
