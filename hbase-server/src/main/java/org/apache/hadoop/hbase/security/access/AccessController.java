...
public class AccessController extends BaseMasterAndRegionObserver
  ...
  private void logResult(AuthResult result) {
    if (result.getRowkey() != null) {
      InetAddress remoteAddr = RpcServer.getRemoteAddress();
      AUDITLOG.info("Access " + (result.isAllowed() ? "allowed" : "denied") +
          ...
          "; rowkey: " + result.getRowkey());
    } else {
      if (AUDITLOG.isDebugEnabled()) {
        InetAddress remoteAddr = RpcServer.getRemoteAddress();
        AUDITLOG.debug("Access " + (result.isAllowed() ? "allowed" : "denied") +
            " for user " + (result.getUser() != null ? result.getUser().getShortName() : "UNKNOWN") +
            "; reason: " + result.getReason() +
            "; remote address: " + (remoteAddr != null ? remoteAddr : "") +
            "; request: " + result.getRequest() +
  ...
  public void preCloneSnapshot(final ObserverContext<MasterCoprocessorEnvironment> ctx,
    ...
    if (SnapshotDescriptionUtils.isSnapshotOwner(snapshot, user)) {
      ...
      AuthResult result = AuthResult.allow("clone" + snapshot.getName(),
  ...
  private void internalPreRead(final ObserverContext<RegionCoprocessorEnvironment> c,
    ...
    case EXISTS:
      ...
      rowkey = Bytes.toString(((Get)query).getRow());
      break;
    case SCAN:
      families = ((Scan)query).getFamilyMap();
      rowkey = Bytes.toString(((Scan)query).getStartRow());
      break;
    default:
    ...
    if (env.getRegionInfo().isMetaTable()) {
      authResult.setRowkey(rowkey == null ? "NONE" : rowkey);
    }
  ...
  @Override
  public void preRefreshGroupServers(final ObserverContext<MasterCoprocessorEnvironment> ctx) throws IOException {
    requirePermission("refreshGroups", Action.ADMIN);
  }

  @Override
  public void preMoveTableToGroup(ObserverContext<MasterCoprocessorEnvironment> ctx, TableName tableName,
      String groupName) throws IOException {
    requirePermission("moveTableToGroup", Action.ADMIN);
  }
}
