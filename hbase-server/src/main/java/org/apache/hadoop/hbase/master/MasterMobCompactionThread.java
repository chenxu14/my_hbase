...
public class MasterMobCompactionThread {
  ...
  public void requestMobCompaction(Configuration conf, FileSystem fs, TableName tableName,
      List<HColumnDescriptor> columns, TableLockManager tableLockManager, boolean allFiles, String date) throws IOException {
    master.reportMobCompactionStart(tableName);
    try {
      masterMobPool.execute(new CompactionRunner(fs, tableName, columns,
        tableLockManager, allFiles, date, mobCompactorPool));
  ...
  private class CompactionRunner implements Runnable {
    ...
    public CompactionRunner(FileSystem fs, TableName tableName, List<HColumnDescriptor> hcds,
      TableLockManager tableLockManager, boolean allFiles, String date, ExecutorService pool) {
      super();
      this.fs = fs;
    ...
    public void run() {
      ...
      try {
        for (HColumnDescriptor hcd : hcds) {
          MobUtils.doMobCompaction(conf, fs, tableName, hcd, pool, allFiles, date, tableLockManager);
        }
      } catch (IOException e) {
