...
public abstract class RpcServer implements RpcServerInterface,
  ...
  protected final List<ServerRpcConnection> connectionList =
    Collections.synchronizedList(new LinkedList<ServerRpcConnection>());
  ...
  public RpcServer(final Server server, final String name,
    ...
    this.bbAllocator = ByteBuffAllocator.create(conf,
        conf.getBoolean("hbase.ipc.server.reservoir.enabled", true));
    this.server = server;
    ...
    this.registry = ((MetricsHBaseServerSourceImpl) metrics.getMetricsSource()).getMetricsRegistry();
  ...
  public Pair<Message, CellScanner> call(RpcCall call,
    ...
    try {
      ...
      updateTableMetrics(param, requestSize, responseSize);
  ...
  private void updateTableMetrics(Message param, long requestSize, long responseSize) {
    try {
      String regionName = null;
      String metricType = null;
      if (param instanceof GetRequest) {
        regionName = Bytes.toStringBinary(((GetRequest) param).getRegion().getValue().toByteArray());
        metricType = "Get";
      } else if (param instanceof MultiRequest) {
        MultiRequest mr = (MultiRequest) param;
        if (mr.getRegionActionCount() > 0) {
          regionName = Bytes
              .toStringBinary(mr.getRegionActionList().get(0).getRegion().getValue().toByteArray());
          metricType = "Multi";
        }
      } else if (param instanceof MutateRequest) {
        regionName = Bytes.toStringBinary(((MutateRequest) param).getRegion().getValue().toByteArray());
        metricType = "Mutate";
      } else if (param instanceof CoprocessorServiceRequest) {
        regionName = Bytes.toStringBinary(((CoprocessorServiceRequest) param).getRegion().getValue().toByteArray());
        metricType = "CP";
      } else if (param instanceof BulkLoadHFileRequest) {
        regionName = Bytes.toStringBinary(((BulkLoadHFileRequest) param).getRegion().getValue().toByteArray());
        metricType = "BulkLoad";
      }
      if (regionName != null && !"".equals(regionName.trim())) {
        String tableName = regionName.split(",")[0];
        tableName = tableName.replace(":", "_"); // split ns and table
        StringBuilder sb = new StringBuilder(tableName).append("_metric_").append(metricType);
        registry.getCounter(sb.toString() + "RequestSize", 0L).incr(requestSize);
        registry.getCounter(sb.toString() + "ResponseSize", 0L).incr(responseSize);
      }
    } catch (Throwable t) {
      LOG.error("update table metrics error!", t);
    }
  }
