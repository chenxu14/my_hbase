...
public class StripeCompactionPolicy extends CompactionPolicy {
  ...
  public StripeCompactionRequest createEmptyRequest(
    ...
    int splitCount = this.config.getInitialCount();
    long targetKvs = estimateTargetKvs(request.getFiles(), splitCount);
    return new SplitStripeCompactionRequest(
        request, OPEN_KEY, OPEN_KEY, splitCount, targetKvs);
  }
  ...
  public StripeCompactionRequest selectCompaction(StripeInformationProvider si,
      List<StoreFile> filesCompacting, boolean isOffpeak, boolean forceMajor) throws IOException {
    ...
    if (StoreUtils.hasReferences(allFiles) || (forceMajor && config.isMajorCompactionEnabled())) {
      LOG.debug("There are references in the store; compacting all files. forceMajor is " + forceMajor);
      long targetKvs = estimateTargetKvs(allFiles, config.getInitialCount());
  ...
  protected StripeCompactionRequest selectSingleStripeCompaction(StripeInformationProvider si,
    ...
    if (hasAllFiles && bqTotalSize >= config.getSplitSize()) {
      ...
      targetCount = config.getSplitCount();
      targetKvs = estimateTargetKvs(filesToCompact, targetCount);
  ...
  private StripeCompactionRequest selectNewStripesCompaction(StripeInformationProvider si) {
    List<StoreFile> l0Files = si.getLevel0Files();
    int targetCount = config.getInitialCount();
    long targetKvs = estimateTargetKvs(l0Files, targetCount);
    if (LOG.isDebugEnabled()) {
      LOG.debug("Creating {}" + targetCount + " initial stripes with "
          + targetKvs + " kvs each via L0 compaction of " + l0Files.size() + " files");
    }
    SplitStripeCompactionRequest request = new SplitStripeCompactionRequest(
        si.getLevel0Files(), OPEN_KEY, OPEN_KEY, targetCount, targetKvs);
  ...
  private long estimateTargetKvs(Collection<StoreFile> files, int splitCount) {
    return getTotalKvCount(files) / splitCount;
  }
