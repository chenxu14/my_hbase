...
public final class FanOutOneBlockAsyncDFSOutputHelper {
  ...
  private static List<Future<Channel>> connectToDataNodes(Configuration conf, DFSClient client,
    ...
    int timeoutMs = conf.getInt(AbstractFSWAL.WAL_SYNC_TIMEOUT, AbstractFSWAL.DEFAULT_WAL_SYNC_TIMEOUT_MS);
      ...
      try {
        ...
        for (int i = 0, n = futureList.size(); i < n; i++) {
          ...
          try {
            ...
            DatanodeInfo dn = locatedBlock.getLocations()[i];
            excludesNodes = ArrayUtils.add(excludesNodes, dn);
            LOG.warn("add dn(" + dn.getHostName() + ") to excludesNodes, current excludesNodes's size is "
                + excludesNodes.length, e);
          }
          if(hasError){
            throw new IOException("some DN alread dead!");
          }
        }
        ...
        LOG.warn("create fan-out dfs output " + src + " failed because of RemoteException, retry = " + retry, e);
        ...
        LOG.warn("create fan-out dfs output " + src + " failed because of NameNodeException, retry = " + retry, e);
        throw e;
      } catch (IOException e) {
        LOG.warn("create fan-out dfs output " + src + " failed because of IOException, retry = " + retry, e);
        if (retry >= createMaxRetries) {
          throw e;
        }
