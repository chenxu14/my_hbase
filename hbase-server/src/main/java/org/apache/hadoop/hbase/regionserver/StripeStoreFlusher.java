...
public class StripeStoreFlusher extends StoreFlusher {
  ...
  protected final StripeCompactionPolicy policy;
  protected final StripeCompactionPolicy.StripeInformationProvider stripes;

  public StripeStoreFlusher(Configuration conf, Store store,
      StripeCompactionPolicy policy, StripeStoreFileManager stripes) throws IOException{
    super(conf, store);
    this.policy = policy;
  ...
  public List<Path> flushSnapshot(MemStoreSnapshot snapshot, long cacheFlushSeqNum,
    ...
    try {
      ...
      synchronized (flushLock) {
        if(this.store instanceof HMobStore){
          performMobFlush(snapshot, cacheFlushSeqNum, scanner, mw, status);
        } else {
  ...
  protected StripeMultiFileWriter.WriterFactory createWriterFactory(final long kvCount) {
