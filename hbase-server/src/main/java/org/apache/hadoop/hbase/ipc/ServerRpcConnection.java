...
abstract class ServerRpcConnection implements Closeable {
  ...
  public void saslReadAndProcess(ByteBuff saslToken) throws IOException,
    ...
    if (saslContextEstablished) {
      RpcServer.LOG.trace("Read input token of size=" + saslToken.limit()
      ...
      try {
        if (saslServer == null) {
          saslServer =
              new HBaseSaslRpcServer(authMethod, rpcServer.secretManager);
          RpcServer.LOG.debug("Created SASL server with mechanism=" + authMethod.getMechanismName());
        }
        RpcServer.LOG.debug("Read input token of size=" + saslToken.limit() + " for processing by saslServer.evaluateResponse()");
  ...
  private void processConnectionHeader(ByteBuff buf) throws IOException {
    ...
    StringBuilder auditInfo = new StringBuilder("Connection from ").append(this.hostAddress)
        .append(":").append(this.remotePort).append(", version=").append(version).append(", sasl=")
        .append(this.useSasl).append(", ugi=").append(this.ugi).append(", service=").append(serviceName);
    RpcServer.AUDITLOG.info(auditInfo.toString());
  }
  ...
  protected void processRequest(ByteBuff buf) throws IOException,
    ...
    try {
      ...
      if (header.hasCellBlockMeta()) {
        ...
        cellScanner = this.rpcServer.ipcUtil.createCellScannerReusingBuffers(
