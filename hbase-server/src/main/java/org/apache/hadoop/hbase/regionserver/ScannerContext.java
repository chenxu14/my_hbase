...
public class ScannerContext {
  ...
  void incrementSizeProgress(long size) {
    long currentSize = progress.getDataSize();
    progress.setDataSize(currentSize + size);
  }
  ...
  void setSizeProgress(long sizeProgress) {
    progress.setDataSize(sizeProgress);
  }
  ...
  boolean checkSizeLimit(LimitScope checkerScope) {
    return hasSizeLimit(checkerScope) && progress.getDataSize() >= limits.getSize();
  }
  ...
  private static class ProgressFields {
     ...
     */
    void setFields(int batch, long dataSize) {
      setBatch(batch);
