...
public class HFileInfo implements SortedMap<byte[], byte[]> {
  private static final Log LOG = LogFactory.getLog(HFileInfo.class);
  ...
  public static boolean isReservedFileInfoKey(byte[] key) {
    return Bytes.startsWith(key, RESERVED_PREFIX_BYTES);
  }
   ...
   */
  public void initMetaAndIndex(HFileReader reader) throws IOException {
