...
public class CompoundBloomFilter extends CompoundBloomFilterBase
   ...
   * @throws IOException
   */
  public CompoundBloomFilter(DataInput meta, HFileReader reader)
      throws IOException {
    this.reader = reader;
  ...
  @Override
  public void warmup() {
    if (index.blockOffsets != null) {
      for (int i = 0; i < index.blockOffsets.length; i++) {
        getBloomBlock(i).release();
      }
    }
  }
