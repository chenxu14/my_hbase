...
public class StochasticLoadBalancer extends BaseLoadBalancer {
  ...
  private float minCostNeedBalance = 0.03f;
  ...
  private LoadCandidateGenerator loadCandidateGenerator;
  ...
  public synchronized void setConf(Configuration conf) {
    ...
    String groupSuffix = "." + this.groupOwner;

    maxSteps = conf.getInt(MAX_STEPS_KEY + groupSuffix, conf.getInt(MAX_STEPS_KEY, maxSteps));
    stepsPerRegion = conf.getInt(STEPS_PER_REGION_KEY + groupSuffix, conf.getInt(STEPS_PER_REGION_KEY, stepsPerRegion));
    maxRunningTime = conf.getLong(MAX_RUNNING_TIME_KEY + groupSuffix, conf.getLong(MAX_RUNNING_TIME_KEY, maxRunningTime));
    numRegionLoadsToRemember = conf.getInt(KEEP_REGION_LOADS + groupSuffix, conf.getInt(KEEP_REGION_LOADS, numRegionLoadsToRemember));
    minCostNeedBalance = conf.getFloat(MIN_COST_NEED_BALANCE_KEY + groupSuffix, conf.getFloat(MIN_COST_NEED_BALANCE_KEY, minCostNeedBalance));

    if (localityCandidateGenerator == null) {
      localityCandidateGenerator = new LocalityBasedCandidateGenerator(services);
    }
    localityCost = new LocalityCostFunction(conf, services, groupSuffix);

    if (loadCandidateGenerator == null) {
      loadCandidateGenerator = new LoadCandidateGenerator();
    }
    ...
    regionLoadFunctions = new CostFromRegionLoadFunction[] {
      new ReadRequestCostFunction(conf, groupSuffix),
      new CPRequestCostFunction(conf, groupSuffix),
      new WriteRequestCostFunction(conf, groupSuffix),
      new MemstoreSizeCostFunction(conf, groupSuffix),
      new StoreFileCostFunction(conf, groupSuffix)
    };

    regionReplicaHostCostFunction = new RegionReplicaHostCostFunction(conf, groupSuffix);
    regionReplicaRackCostFunction = new RegionReplicaRackCostFunction(conf, groupSuffix);
    ...
    costFunctions = new CostFunction[]{
      ...
      localityCost,
      new TableSkewCostFunction(conf, groupSuffix),
    ...
    simpleFunctions = new CostFunction[] {
      new RegionCountSkewCostFunction(conf, groupSuffix),
      new PrimaryRegionCountSkewCostFunction(conf, groupSuffix),
      new MoveCostFunction(conf, groupSuffix),
      regionReplicaHostCostFunction,
  ...
  protected boolean needsBalance(Cluster cluster) {
    ...
    for (CostFunction c : costFunctions) {
      ...
      if (!c.isNeeded()) {
        LOG.info(c.getClass().getName() + " indicated that its cost should not be considered");
        continue;
      }
  ...
  public synchronized List<RegionPlan> balanceCluster(Map<ServerName,
    ...
    if (!mandatory && !needsBalance(cluster)) {
      LOG.info("no need to do balancer, bypass it.");
      return null;
    }

    double currentCost = computeCost(cluster, Double.MAX_VALUE, mandatory);
    ...
    for (step = 0; step < computedMaxSteps; step++) {
      ...
      CandidateGenerator p = mandatory ? loadCandidateGenerator : candidateGenerators[generatorIdx];
    ...
    if (initCost > currentCost) {
      ...
      LOG.info("Finished computing new load balance plan.  Computation took "
          ...
          + (plans == null ? 0 : plans.size()) + " regions; Going from a computed cost of "
  ...
  private String functionCost(boolean mandatory) {
  ...
  protected void initCosts(Cluster cluster, boolean mandatory) {
    for (CostFunction c : mandatory ? simpleFunctions : costFunctions) {
      c.init(cluster);
    }
  }

  protected void updateCostsWithAction(Cluster cluster, Action action, boolean mandatory) {
    for (CostFunction c : mandatory ? simpleFunctions : costFunctions) {
      c.postAction(action);
    }
  }
  ...
  protected double computeCost(Cluster cluster, double previousCost, boolean mandatory) {
    double total = 0;
    CostFunction[] funcs = mandatory ? simpleFunctions : costFunctions;
    for (CostFunction c : funcs) {
  ...
  static class MoveCostFunction extends CostFunction {
    ...
    MoveCostFunction(Configuration conf, String groupSuffix) {
      super(conf);
      ...
      this.setMultiplier(conf.getFloat(MOVE_COST_KEY + groupSuffix, conf.getFloat(MOVE_COST_KEY, DEFAULT_MOVE_COST)));
  ...
  static class RegionCountSkewCostFunction extends CostFunction {
    ...
    RegionCountSkewCostFunction(Configuration conf, String groupSuffix) {
      super(conf);
      // Load multiplier should be the greatest as it is the most general way to balance data.
      this.setMultiplier(conf.getFloat(REGION_COUNT_SKEW_COST_KEY + groupSuffix,
          conf.getFloat(REGION_COUNT_SKEW_COST_KEY, DEFAULT_REGION_COUNT_SKEW_COST)));
    }
  ...
  static class PrimaryRegionCountSkewCostFunction extends CostFunction {
    ...
    PrimaryRegionCountSkewCostFunction(Configuration conf, String groupSuffix) {
      super(conf);
      // Load multiplier should be the greatest as primary regions serve majority of reads/writes.
      this.setMultiplier(conf.getFloat(PRIMARY_REGION_COUNT_SKEW_COST_KEY + groupSuffix,
  ...
  static class TableSkewCostFunction extends CostFunction {
    ...
    TableSkewCostFunction(Configuration conf, String groupSuffix) {
      super(conf);
      this.setMultiplier(conf.getFloat(TABLE_SKEW_COST_KEY + groupSuffix,
          conf.getFloat(TABLE_SKEW_COST_KEY, DEFAULT_TABLE_SKEW_COST)));
    }
  ...
  static class LocalityCostFunction extends CostFunction {
    ...
    LocalityCostFunction(Configuration conf, MasterServices srv, String groupSuffix) {
      super(conf);
      float multiplier = conf.getFloat(LOCALITY_COST_KEY + groupSuffix, conf.getFloat(LOCALITY_COST_KEY, DEFAULT_LOCALITY_COST));
  ...
  static class ReadRequestCostFunction extends CostFromRegionLoadFunction {
    ...
    ReadRequestCostFunction(Configuration conf, String groupSuffix) {
      super(conf);
      float multiplier = conf.getFloat(READ_REQUEST_COST_KEY + groupSuffix,
          conf.getFloat(READ_REQUEST_COST_KEY, DEFAULT_READ_REQUEST_COST));
      this.setMultiplier(multiplier);
      if (LOG.isDebugEnabled()) {
        LOG.debug("ReadRequestCost on " + groupSuffix + " is : " + multiplier);
      }
    }
  ...
  static class CPRequestCostFunction extends CostFromRegionLoadFunction {
    ...
    CPRequestCostFunction(Configuration conf, String groupSuffix) {
      super(conf);
      float multiplier = conf.getFloat(CP_REQUEST_COST_KEY + groupSuffix,
          conf.getFloat(CP_REQUEST_COST_KEY, DEFAULT_CP_REQUEST_COST));
      this.setMultiplier(multiplier);
      if (LOG.isDebugEnabled()) {
        LOG.debug("CPRequestCost on " + groupSuffix + " is : " + multiplier);
      }
    }
  ...
  static class WriteRequestCostFunction extends CostFromRegionLoadFunction {
    ...
    WriteRequestCostFunction(Configuration conf, String groupSuffix) {
      super(conf);
      float multiplier = conf.getFloat(WRITE_REQUEST_COST_KEY + groupSuffix,
          conf.getFloat(WRITE_REQUEST_COST_KEY, DEFAULT_WRITE_REQUEST_COST));
      this.setMultiplier(multiplier);
      if (LOG.isDebugEnabled()) {
        LOG.debug("WriteRequestCost on " + groupSuffix + " is : " + multiplier);
      }
    }
  ...
  static class RegionReplicaHostCostFunction extends CostFunction {
    ...
    public RegionReplicaHostCostFunction(Configuration conf, String groupSuffix) {
      super(conf);
      this.setMultiplier(conf.getFloat(REGION_REPLICA_HOST_COST_KEY + groupSuffix,
  ...
  static class RegionReplicaRackCostFunction extends RegionReplicaHostCostFunction {
    ...
    public RegionReplicaRackCostFunction(Configuration conf, String groupSuffix) {
      super(conf, groupSuffix);
      this.setMultiplier(conf.getFloat(REGION_REPLICA_RACK_COST_KEY + groupSuffix,
  ...
  static class MemstoreSizeCostFunction extends CostFromRegionLoadFunction {
    ...
    MemstoreSizeCostFunction(Configuration conf, String groupSuffix) {
      super(conf);
      float multiplier = conf.getFloat(MEMSTORE_SIZE_COST_KEY + groupSuffix,
          conf.getFloat(MEMSTORE_SIZE_COST_KEY, DEFAULT_MEMSTORE_SIZE_COST));
      this.setMultiplier(multiplier);
      if (LOG.isDebugEnabled()) {
        LOG.debug("MemstoreSizeCost on " + groupSuffix + " is : " + multiplier);
      }
    }
  ...
  static class StoreFileCostFunction extends CostFromRegionLoadFunction {
    ...
    StoreFileCostFunction(Configuration conf, String groupSuffix) {
      super(conf);
      float multiplier = conf.getFloat(STOREFILE_SIZE_COST_KEY + groupSuffix,
          conf.getFloat(STOREFILE_SIZE_COST_KEY, DEFAULT_STOREFILE_SIZE_COST));
      this.setMultiplier(multiplier);
      if (LOG.isDebugEnabled()) {
        LOG.debug("StoreFileCost on " + groupSuffix + " is : " + multiplier);
      }
    }
