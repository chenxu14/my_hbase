...
class MemStoreFlusher implements FlushRequester {
  ...
  private boolean flushOneForGlobalPressure() {
    SortedMap<Long, Region> regionsBySize = null;
    switch(flushType) {
    ...
    if (!region.getRegionInfo().isMetaRegion() &&
      ...
      if (fqe.isMaximumWait(this.blockingWaitTime)) {
        ...
        if (fqe.getRequeueCount() <= 0) {
          ...
          LOG.warn(region.getRegionInfo().getEncodedName() + " has too many store files(" +
              getStoreFileCount(region) +"); delaying flush up to "+ this.blockingWaitTime + " ms");
