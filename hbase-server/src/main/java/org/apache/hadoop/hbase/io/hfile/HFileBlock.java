...
public class HFileBlock implements Cacheable {
   ...
   */
  @VisibleForTesting
  public HFileBlock(BlockType blockType, int onDiskSizeWithoutHeader, int uncompressedSizeWithoutHeader,
      long prevBlockOffset, ByteBuff buf, boolean fillHeader, long offset,
      int nextBlockOnDiskSize, int onDiskDataSizeWithHeader, HFileContext fileContext, ByteBuffAllocator allocator) {
  ...
  static HFileBlock createFromBuff(ByteBuff buf, boolean usesHBaseChecksum, final long offset,
      final int nextBlockOnDiskSize, HFileContext fileContext, ByteBuffAllocator allocator) throws IOException {
    buf.rewind();
   ...
   * used in {@link org.apache.hadoop.hbase.io.hfile.CompoundBloomFilter}
  ...
  static class Writer {
    ...
    void beforeShipped() {
      if (dataBlockEncodingCtx.getEncodingState() != null) {
        dataBlockEncodingCtx.getEncodingState().beforeShipped();
      }
    }
     ...
     */
    ByteBuff cloneUncompressedBuffWithHeader() {
      expectState(State.BLOCK_READY);
    ...
    HFileBlock getBlockForCaching(CacheConfig cacheConf) {
      ...
      if (cacheConf.shouldCacheCompressed(blockType.getCategory())) {
        ...
        buff = cloneUncompressedBuffWithHeader();
      }
  ...
  static class FSReaderImpl implements FSReader {
    ...
    public BlockIterator blockRange(final long startOffset, final long endOffset) {
      ...
      return new BlockIterator() {
        ...
        public HFileBlock nextBlock() throws IOException {
          ...
          HFileBlock b = readBlockData(offset, length, false, false, false);
    ...
    public HFileBlock readBlockData(long offset, long onDiskSizeWithHeaderL,
      ...
      HFileBlock blk = readBlockDataInternal(is, offset, onDiskSizeWithHeaderL,
          pread, doVerificationThruHBaseChecksum, updateMetrics, intoHeap);
      if (blk == null) {
    ...
    protected HFileBlock readBlockDataInternal(FSDataInputStream is, long offset,
        long onDiskSizeWithHeaderL, boolean pread,
        boolean verifyChecksum, boolean updateMetrics, boolean intoHeap)
    throws IOException {
      if (offset < 0) {
      ...
      try {
        ...
        HFileBlock hFileBlock = createFromBuff(curBlock, this.fileContext.isUseHBaseChecksum(), offset,
