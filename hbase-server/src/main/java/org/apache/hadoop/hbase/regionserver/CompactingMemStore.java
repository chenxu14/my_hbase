...
public class CompactingMemStore extends AbstractMemStore {
  ...
  void inMemoryCompaction() {
    ...
    try {
      ...
      LOG.warn("Unable to run in-memory compaction on " + 
          getRegionServices().getRegionInfo().getEncodedName() + "/"+
          getFamilyName() + "; exception=" + e);
    }
  }
  ...
  protected boolean shouldFlushInMemory(MutableSegment currActive, Cell cellToAdd,
    ...
    if(LOG.isDebugEnabled()) {
      LOG.debug("size above flush threshold, cellToAdd size is " + cellSize + ", segmentDataSize is " + segmentDataSize);
    }
