...
public class StripeStoreConfig {
  ...
  public static final String MAJOR_COMPACTION_ENABLED = "hbase.store.stripe.majorcompaction.enabled";
  ...
  private final boolean majorEnabled;
  ...
  public StripeStoreConfig(Configuration config, StoreConfigInformation sci) {
    ...
    this.majorEnabled = config.getBoolean(MAJOR_COMPACTION_ENABLED, false);
    ...
    this.maxRegionSplitImbalance = config.getFloat(MAX_REGION_SPLIT_IMBALANCE_KEY, 1.5f);
    int splitPartCount = config.getInt(SPLIT_PARTS_KEY, 2);
  ...
  public boolean isMajorCompactionEnabled() {
    return majorEnabled;
  }
