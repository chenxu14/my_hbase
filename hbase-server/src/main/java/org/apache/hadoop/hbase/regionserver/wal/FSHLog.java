...
public class FSHLog extends AbstractFSWAL<Writer> {
  ...
  protected Writer createWriterInstance(final Path path, final Path oldPath) throws IOException {
    Writer writer = FSHLogProvider.createWriter(conf, fs, path, oldPath, false);
  ...
  @Override
  protected long append(final HRegionInfo hri, final WALKey key, final WALEdit edits,
  ...
  private class SyncRunner extends HasThread {
    ...
    private int releaseSyncFutures(final long currentSequence, final Throwable t) {
      ...
      for (SyncFuture syncFuture; (syncFuture = this.syncFutures.peek()) != null;) {
        if (syncFuture.getTxid() > currentSequence) break;
      ...
      while (!isInterrupted()) {
        ...
        try {
          ...
          try {
            ...
            if (lastException != null) requestLogRoll(false, true);
            else checkLogRoll();
          }
