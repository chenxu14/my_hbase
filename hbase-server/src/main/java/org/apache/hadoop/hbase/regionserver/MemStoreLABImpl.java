...
public class MemStoreLABImpl implements MemStoreLAB {
  ...
  public Cell copyCellInto(Cell cell) {
    ...
    return (cell instanceof ByteBufferKeyValue)?
        copyBBECellInto((ByteBufferKeyValue)cell, maxAlloc):
   ...
   */
  private Cell copyBBECellInto(ByteBufferKeyValue cell, int maxAlloc) {
    int size = cell.getSerializedSize(true);
    Preconditions.checkArgument(size >= 0, "negative size");
   ...
   */
  private static Cell copyBBECToChunkCell(ByteBufferKeyValue cell, ByteBuffer buf, int offset,
      int len) {
    int tagsLen = cell.getTagsLength();
  ...
  private static Cell createChunkCell(ByteBuffer buf, int offset, int len, int tagsLen,
    ...
    if (tagsLen == 0) {
      ...
      // call getTagsLength().
      return new NoTagByteBufferChunkCell(buf, offset, len, sequenceId);
    } else {
      return new ByteBufferChunkCell(buf, offset, len, sequenceId);
    }
  }
