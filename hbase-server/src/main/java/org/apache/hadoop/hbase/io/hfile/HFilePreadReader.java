...
/**
 * Implementation of {@link HFileReader} to deal with pread.
 */
@InterfaceAudience.Private
public class HFilePreadReader extends HFileReaderImpl {
  private static final Log LOG = LogFactory.getLog(HFilePreadReader.class);

  public HFilePreadReader(ReaderContext context, HFileInfo fileInfo,
      CacheConfig cacheConf, Configuration conf) throws IOException {
    super(context, fileInfo, cacheConf, conf);
    // Prefetch file blocks upon open if requested
    if (cacheConf.shouldPrefetchOnOpen(context.getFileSize())) {
    ...
    } else if (cacheConf.shouldPrefetchIndex()) {
      if (LOG.isDebugEnabled()) {
        LOG.debug("Prefetch index only, " + context.getFilePath());
      }
      PrefetchExecutor.request(path, new Runnable() {
        public void run() {
          try {
            dataBlockIndexReader.warmup(
                dataBlockEncoder.getEffectiveEncodingInCache(false), path, HFilePreadReader.this);
          } catch (Exception e) {
            LOG.warn("File=" + path.toString() + ", do warmup failed!", e);
          } finally {
            PrefetchExecutor.complete(path);
          }
        }
