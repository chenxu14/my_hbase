...
import org.apache.hadoop.hbase.master.LogRecoveryManager.TerminationStatus;
...
public class ZKSplitLogManagerCoordination extends ZooKeeperListener implements
  ...
  private void rescan(long retries) {
    ...
    SplitLogTask slt = new SplitLogTask.Done(this.details.get(Type.HDFS).getServerName(), getRecoveryMode());
    this.watcher
  ...
  public void markRegionsRecovering(final ServerName serverName, Set<HRegionInfo> userRegions)
    ...
    for (HRegionInfo region : userRegions) {
      ...
      do {
        ...
        try {
          long lastSequenceId =
              this.details.get(Type.HDFS).getMaster().getServerManager()
