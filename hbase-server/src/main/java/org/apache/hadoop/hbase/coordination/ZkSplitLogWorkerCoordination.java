...
public class ZkSplitLogWorkerCoordination extends ZooKeeperListener implements
   ...
   */
  private boolean grabTask(String taskName) {
    Stat stat = new Stat();
    byte[] data;
    synchronized (grabTaskLock) {
    ...
    try {
      try {
        if ((data = ZKUtil.getDataNoWatch(watcher, taskName, stat)) == null) {
          SplitLogCounters.tot_wkr_failed_to_grab_task_no_data.incrementAndGet();
          return false;
        }
      } catch (KeeperException e) {
        LOG.warn("Failed to get data for znode " + taskName, e);
      ...
      try {
        ...
        LOG.warn("Failed parse data for znode " + taskName, e);
        SplitLogCounters.tot_wkr_failed_to_grab_task_exception.incrementAndGet();
        return false;
      }
      if (!slt.isUnassigned()) {
      ...
      currentVersion =
          attemptToOwnTask(true, watcher, server.getServerName(), taskName,
      ...
      LOG.info("worker " + server.getServerName() + " acquired task " + taskName);
      SplitLogCounters.tot_wkr_task_acquired.incrementAndGet();
      getDataSetWatchAsync();

      submitTask(taskName, slt.getMode(), currentVersion, reportPeriod);
  ...
  public void taskLoop() throws InterruptedException {
    while (!shouldStop) {
      ...
      int offset = (int) (Math.random() * tasks.size());
      for (int i = 0; i < tasks.size(); i++) {
        if (this.worker.isMetaTask(tasks.get(i))) {
          offset = i;
          break;
        }
      }
