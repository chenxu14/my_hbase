...
public class HFilePrettyPrinter extends Configured implements Tool {
  ...
  public int processFile(Path file) throws IOException {
    ...
    HFileReader reader = HFile.createReader(fs, file, new CacheConfig(getConf()), true, getConf());
      ...
      if (printKey) {
        ...
        if (printValue) {
          ...
          List<Tag> tags = TagUtil.asList(cell.getTagsArray(), cell.getTagsOffset(),
              cell.getTagsLength());
          for (Tag tag : tags) {
            out.print(String.format(" T[%d]: %s", i++, TagUtil.getValueAsString(tag)));
          }
        }
        out.println();
      }
  ...
  private void printMeta(HFileReader reader, Map<byte[], byte[]> fileInfo)
      throws IOException {
    ...
    for (Map.Entry<byte[], byte[]> e : fileInfo.entrySet()) {
      ...
      } else if (Bytes.compareTo(e.getKey(), HFileInfo.AVG_KEY_LEN) == 0
          || Bytes.compareTo(e.getKey(), HFileInfo.AVG_VALUE_LEN) == 0) {
  ...
  private static class KeyValueStatsCollector {
    ...
    private void collectRow() {
      ...
      if (curRowBytes > maxRowBytes && prevCell != null) {
        biggestRow = prevCell.getRowArray();
