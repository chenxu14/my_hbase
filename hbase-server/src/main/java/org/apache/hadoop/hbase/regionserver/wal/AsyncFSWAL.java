...
public class AsyncFSWAL extends AbstractFSWAL<AsyncWriter> {
  ...
  private final int preallocatedEventCount;
  ...
  public AsyncFSWAL(FileSystem fs, Path rootDir, String logDir, String archiveDir,
      ...
      String prefix, String suffix, EventLoopGroup eventLoopGroup, Class<? extends Channel> channelClass)
  ...
  private void syncFailed(long epochWhenSync, Throwable error) {
    ...
    try {
      ...
      this.epochAndState = currentEpochAndState | 0b10; // mark as broken
  ...
  private void appendAndSync() {
    ...
    if (writer.getLength() - fileLengthAtLastSync >= batchSize) {
      ...
      if (LOG.isDebugEnabled()) {
        LOG.debug("sync because buffer size limit " + batchSize);
      }
      sync(writer);
      return;
    }
  ...
  private void consume() {
    ...
    try {
      int currentEpochAndState = epochAndState;
      if (writerBroken(currentEpochAndState)) { // sync fail has happen
        return;
      }
      if (waitingRoll(currentEpochAndState)) {
        if (writer.getLength() > fileLengthAtLastSync) {
          // issue a sync, flush all uncommited change
          if (LOG.isDebugEnabled()) {
            LOG.debug("issue a sync to flush all uncommited change.");
          }
          sync(writer);
        } else {
          if (unackedAppends.isEmpty()) {
            ...
            readyForRollingCond.signalAll();
            if (LOG.isDebugEnabled()) {
              LOG.debug("unackedAppends is Empty, ready for rolling!");
            }
          }
        }
        return;
    ...
    long curCursor = waitingConsumePayloadsGatingSequence.get();
    long cursorBound = waitingConsumePayloads.getCursor();
    long diff = cursorBound - curCursor;
    try { // add debug info
      if (diff > preallocatedEventCount * 0.5) {
        LOG.warn(getCurrentFileName().toString() + ", ringbuffer's size is " + diff + ", consumer blocked?");
      }
      if(LOG.isTraceEnabled()) {
        LOG.trace(getCurrentFileName().toString() + ": " + diff + " RingBufferTrucks to process");
      }
    } catch (Exception e) {}

    for (long nextCursor = curCursor + 1; nextCursor <= cursorBound; nextCursor++) {
      ...
      if (waitingConsumePayloadsGatingSequence.get() == waitingConsumePayloads.getCursor()) {
        ...
        if (waitingConsumePayloadsGatingSequence.get() == waitingConsumePayloads.getCursor()) {
          ...
          if (writer.getLength() > fileLengthAtLastSync && !syncFutures.isEmpty() &&
            ...
            if (LOG.isDebugEnabled()) {
              LOG.debug("no new data in the ringbuffer and we have at least one sync request.");
            }
            sync(writer);
          }
          return;
    ...
    if (writerBroken(currentEpochAndState) || waitingRoll(currentEpochAndState)) {
      if(LOG.isDebugEnabled()) {
        if (writerBroken(currentEpochAndState)) {
          LOG.debug("current writer is Broken, bypass Consumer schedule.");
        } else {
          LOG.debug("current writer is waiting for roll, bypass Consumer schedule.");
        }
      }
      return false;
    }
  ...
  @Override
  public long append(HTableDescriptor htd, HRegionInfo hri, WALKey key, WALEdit edits, boolean inMemstore)
      throws IOException {
    long txid =
  ...
  protected AsyncWriter createWriterInstance(Path path, Path oldPath) throws IOException {
    return AsyncFSWALProvider.createAsyncWriter(conf, fs, path, oldPath, false, eventLoopGroup, channelClass);
  }
  ...
  private void waitForSafePoint() {
    ...
    try {
      ...
      consumerScheduled.set(true); // consumer will not scheduled after this, when do append and sync
      epochAndState = currentEpochAndState | 1; // mark as waiting roll
  ...
  protected void doReplaceWriter(Path oldPath, Path newPath, AsyncWriter nextWriter)
    ...
    if (LOG.isDebugEnabled()) {
      LOG.debug("do replace writer cost " + (EnvironmentEdgeManager.currentTime() - start) + " ms.");
    }
  }
