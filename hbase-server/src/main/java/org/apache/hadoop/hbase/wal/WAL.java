...
public interface WAL extends Closeable, WALFileLengthProvider {
   ...
   *         region names as returned by {@link HRegionInfo#getEncodedName()}
   */
  byte[][] rollWriter(boolean force, boolean syncFailed) throws IOException;
   ...
   */
  long appendData(HRegionInfo info, WALKey key, WALEdit edits) throws IOException;
   ...
   */
  long appendMarker(HRegionInfo info, WALKey key, WALEdit edits) throws IOException;
