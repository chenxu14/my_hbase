...
public abstract class ServerCall<T extends ServerRpcConnection> implements RpcCall, RpcResponse {
  ...
  protected final IPCUtil cellBlockBuilder;
  ...
  ServerCall(int id, BlockingService service, MethodDescriptor md, RequestHeader header,
      ...
      IPCUtil cellBlockBuilder, CallCleanup reqCleanup) {
    this.id = id;
  ...
  public void addResultSize(Result result) {
    if (result != null && !result.isEmpty()) {
      for (Cell c : result.rawCells()) {
        this.responseCellSize.addAndGet(CellUtil.estimatedSerializedSizeOf(c));
      }
    }
  }

  @Override
  public boolean isAboveQuota(long maxQuotaResultSize, long maxRowsPerMultiGet) {
    return isRetryImmediatelySupported()
        && (getResponseCellSize() > maxQuotaResultSize || getNumsOfGet() > maxRowsPerMultiGet);
  }
