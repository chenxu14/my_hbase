...
import org.apache.hadoop.hbase.shaded.protobuf.generated.ClientProtos.Scan;
...
public class AnnotationReadingPriorityFunction implements PriorityFunction {
  ...
  private CoprocessorJudger judger;
  ...
  public AnnotationReadingPriorityFunction(final RSRpcServices rpcServices,
    ...
    // if Client call ResultScanner.next() more than maxSeq, think it as a large query
    maxSeq = conf.getLong("hbase.ipc.server.largequery.maxseq", 20);
    offpeak = OffPeakHours.getInstance(conf);
    Class<? extends CoprocessorJudger> judgerClass =  conf.getClass(
        "hbase.regionserver.largequery.judger.class", null, CoprocessorJudger.class); 
    if (judgerClass != null) {
      judger = ReflectionUtils.newInstance(judgerClass);
    }
  }
  ...
  protected int getBasePriority(RequestHeader header, Message param) {
    ...
    boolean isOffpeakHour = offpeak.isOffPeakHour();
    if (param instanceof ScanRequest) { // scanner methods...
      ScanRequest request = (ScanRequest)param;
      Scan scan = request.getScan();
      boolean isLargeQuery = false;
      boolean hasLargeQuery = false;
      for (NameBytesPair attribute: scan.getAttributeList()) {
        if(attribute.getName().equals("LARGEQUERY")) {
          hasLargeQuery = true;
          byte[] value = attribute.getValue().toByteArray();
          if(value != null) {
            try {
              isLargeQuery = Bytes.toBoolean(value);
      ...
      long seqId = request.getNextCallSeq();
      if(!isOffpeakHour && (seqId > maxSeq || isLargeQuery)) {
        return HConstants.LARGEQUERY_QOS;
      }

      if (!request.hasScannerId()) {
        // request in offpeak hours, return NORMAL_QOS
        if (isOffpeakHour) {
          return HConstants.NORMAL_QOS;
        }
        String startRow = scan.getStartRow().toStringUtf8();
        String stopRow = scan.getStopRow().toStringUtf8();
        if (!hasLargeQuery && ("".equals(startRow) || "".equals(stopRow))) {
          return HConstants.LARGEQUERY_QOS;
        }
        return HConstants.NORMAL_QOS;
      }
      ...
      if (!isOffpeakHour && scanner != null && scanner.isLargeQuery()){
        return HConstants.LARGEQUERY_QOS;
      }
    }

    if (judger != null && param instanceof CoprocessorServiceRequest) {
      CoprocessorServiceRequest request = (CoprocessorServiceRequest)param;
      CoprocessorServiceCall call = request.getCall();
      if (judger.isLargeQuery(call.getRequest())){
        return HConstants.LARGEQUERY_QOS;
      }
    }
    return HConstants.NORMAL_QOS;
  ...
  public void setConf(Configuration conf) {
    scanVirtualTimeWeight = conf.getFloat(SCAN_VTIME_WEIGHT_CONF_KEY, 1.0f);
    maxSeq = conf.getLong("hbase.ipc.server.largequery.maxseq", 20);
    LOG.info("hbase.ipc.server.largequery.maxseq changed to " + maxSeq);
    offpeak = OffPeakHours.getInstance(conf);
  }
}
