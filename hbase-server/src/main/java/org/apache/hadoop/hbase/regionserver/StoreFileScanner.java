...
public class StoreFileScanner implements KeyValueScanner {
  private static final Recycler<StoreFileScanner> RECYCLER = new Recycler<StoreFileScanner>() {
    @Override
    protected StoreFileScanner newObject(Handle<StoreFileScanner> handle) {
      return new StoreFileScanner(handle);
    }
  };
  ...
  private boolean canOptimizeForNonNullColumn;
  ...
  private Handle<StoreFileScanner> handle;
   ...
   */
  public static StoreFileScanner newInstance(StoreFile.Reader reader, HFileScanner hfs, boolean useMVCC,
      boolean hasMVCC, long readPt, long scannerOrder, boolean canOptimizeForNonNullColumn) {
    StoreFileScanner scanner = RECYCLER.get();
    scanner.readPt = readPt;
    scanner.reader = reader;
    scanner.hfs = hfs;
    scanner.enforceMVCC = useMVCC;
    scanner.hasMVCCInfo = hasMVCC;
    scanner.scannerOrder = scannerOrder;
    scanner.canOptimizeForNonNullColumn = canOptimizeForNonNullColumn;
    scanner.closed = false;
    scanner.realSeekDone = false;
    scanner.delayedReseek = false;
    scanner.stopSkippingKVsIfNextRow = false;
    return scanner;
  }
  ...
  private StoreFileScanner(Handle<StoreFileScanner> handle) {
    this.handle = handle;
  }
