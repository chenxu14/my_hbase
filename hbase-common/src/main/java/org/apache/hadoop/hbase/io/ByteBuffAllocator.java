...
public class ByteBuffAllocator {
  ...
  public static ByteBuffAllocator create(Configuration conf, boolean reservoirEnabled) {
    ...
    if (reservoirEnabled) {
      ...
      LOG.warn("hbase.ipc.server.reservoir.enabled is false");
      return HEAP;
    }
  }
  ...
  private ByteBuffer allocateOnHeap(int size) {
    if (size != HConstants.HFILEBLOCK_HEADER_SIZE) {
      printTrack(isReservoirEnabled(), size);
    }
    heapAllocationBytes.add(size);
    return ByteBuffer.allocate(size);
  }

  private void printTrack(boolean reservoirEnabled, int size) {
    LOG.info("reservoirEnabled : " + reservoirEnabled + ", size : " + size);
