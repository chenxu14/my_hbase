...
public final class CellUtil {
  ...
  public static int copyQualifierTo(Cell cell, ByteBuffer destination, int destinationOffset) {
    int qlen = cell.getQualifierLength();
    if (cell instanceof ByteBufferExtendedCell) {
      ByteBufferUtils.copyFromBufferToBuffer(((ByteBufferExtendedCell) cell).getQualifierByteBuffer(),
          destination, ((ByteBufferExtendedCell) cell).getQualifierPosition(), destinationOffset, qlen);
    } else {
  ...
  public static byte getRowByte(Cell cell, int index) {
    if (cell instanceof ByteBufferExtendedCell) {
      return ((ByteBufferExtendedCell) cell).getRowByteBuffer().get(
          ((ByteBufferExtendedCell) cell).getRowPosition() + index);
    }
  ...
  public static byte getQualifierByte(Cell cell, int index) {
    if (cell instanceof ByteBufferExtendedCell) {
      return ((ByteBufferExtendedCell) cell).getQualifierByteBuffer().get(
          ((ByteBufferExtendedCell) cell).getQualifierPosition() + index);
    }
  ...
  @InterfaceAudience.Private
  private static class TagRewriteByteBufferExtendedCell extends ByteBufferExtendedCell {
  ...
  @InterfaceAudience.Private
  private static class ValueAndTagRewriteByteBufferExtendedCell extends TagRewriteByteBufferExtendedCell {
    ...
    public Cell deepClone() {
      Cell clonedBaseCell = ((ExtendedCell) this.cell).deepClone();
      if (clonedBaseCell instanceof ByteBufferExtendedCell) {
        return new ValueAndTagRewriteByteBufferExtendedCell((ByteBufferExtendedCell) clonedBaseCell, this.value,
            this.tags);
      }
      return new ValueAndTagRewriteCell(clonedBaseCell, this.value, this.tags);
    }
  ...
  public static boolean matchingRow(final Cell left, final byte[] buf, final int offset,
    ...
    if (left instanceof ByteBufferExtendedCell) {
      return ByteBufferUtils.equals(((ByteBufferExtendedCell) left).getRowByteBuffer(),
          ((ByteBufferExtendedCell) left).getRowPosition(), left.getRowLength(), buf, offset,
          length);
    }
  ...
  public static boolean matchingFamily(final Cell left, final byte[] buf, final int offset,
    ...
    if (left instanceof ByteBufferExtendedCell) {
      return ByteBufferUtils.equals(((ByteBufferExtendedCell) left).getFamilyByteBuffer(),
          ((ByteBufferExtendedCell) left).getFamilyPosition(), left.getFamilyLength(), buf,
          offset, length);
    }
  ...
  public static boolean matchingColumnFamilyAndQualifierPrefix(final Cell left, final byte[] fam,
      final byte[] qual) {
    return matchingFamily(left, fam) && CellUtil.qualifierStartsWith(left, qual);
  }
  ...
  public static int findCommonPrefixInFlatKey(Cell c1, Cell c2, boolean bypassFamilyCheck,
    ...
    if (c1 instanceof ByteBufferExtendedCell && c2 instanceof ByteBufferExtendedCell) {
      rkCommonPrefix = ByteBufferUtils.findCommonPrefix(((ByteBufferExtendedCell) c1).getRowByteBuffer(),
        ((ByteBufferExtendedCell) c1).getRowPosition(), rLen1, ((ByteBufferExtendedCell) c2).getRowByteBuffer(),
  ...
  public static Cell createFirstOnRowFamily(Cell cell, byte[] fArray, int foff, int flen) {
    if (cell instanceof ByteBufferExtendedCell) {
      return new FirstOnRowColByteBufferExtendedCell(((ByteBufferExtendedCell) cell).getRowByteBuffer(),
          ((ByteBufferExtendedCell) cell).getRowPosition(), cell.getRowLength(), ByteBuffer.wrap(fArray),
  ...
  private static class FirstOnRowCell extends EmptyCell {
    ...
    public long heapSize() {
      return ClassSize.align(FIXED_HEAPSIZE)
          // array overhead
          + (rlength == 0 ? ClassSize.sizeOf(rowArray, rlength) : rlength);
    }
  ...
  private static class FirstOnRowColCell extends FirstOnRowCell {
    ...
    public long heapSize() {
      return ClassSize.align(FIXED_HEAPSIZE)
          // array overhead
          + (flength == 0 ? ClassSize.sizeOf(fArray, flength) : flength)
          + (qlength == 0 ? ClassSize.sizeOf(qArray, qlength) : qlength);
    }
  ...
  private static class LastOnRowColCell extends LastOnRowCell {
    private static final int FIXED_OVERHEAD = LastOnRowCell.FIXED_OVERHEAD
  ...
  private static class FirstOnRowDeleteFamilyCell extends EmptyCell {
    ...
    public long heapSize() {
      return ClassSize.align(FIXED_OVERHEAD)
        // array overhead
        + (getRowLength() == 0 ? ClassSize.sizeOf(row, getRowLength()) : getRowLength())
        + (getFamilyLength() == 0 ?
          ClassSize.sizeOf(fam, getFamilyLength()) : getFamilyLength());
    }
