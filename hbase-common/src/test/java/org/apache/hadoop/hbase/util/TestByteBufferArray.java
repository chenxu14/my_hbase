...
public class TestByteBufferArray {
  ...
  private static final ByteBufferAllocator ALLOC = (size,direct) -> 
      direct ? ByteBuffer.allocateDirect((int) size) : ByteBuffer.allocate((int) size);

  @Test
  public void testAsSubBufferWhenEndOffsetLandInLastBuffer() throws Exception {
    int capacity = 4 * 1024 * 1024;
    ByteBufferArray array = new ByteBufferArray(capacity, false, ALLOC);
