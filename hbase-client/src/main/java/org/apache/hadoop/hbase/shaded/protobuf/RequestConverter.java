...
public final class RequestConverter {
  ...
  public static BulkLoadHFileRequest buildBulkLoadHFileRequest(
    ...
    for (Pair<byte[], String> familyPath: familyPaths) {
      familyPathBuilder.setFamily(UnsafeByteOperations.unsafeWrap(familyPath.getFirst()));
    ...
    for (Action<R> action: actions) {
      ...
      if (row instanceof Get) {
        ...
        builder.addAction(actionBuilder.setServiceCall(ClientProtos.CoprocessorServiceCall
            .newBuilder().setRow(UnsafeByteOperations.unsafeWrap(exec.getRow()))
            .setServiceName(exec.getMethod().getService().getFullName())
            .setMethodName(exec.getMethod().getName())
            .setRequest(value)));
      } else if (row instanceof RowMutations) {
...
//Start utilities for Admin
 ...
 public static CompactRegionRequest buildCompactRegionRequest(
     final byte[] regionName, final boolean major, final byte [] family, String date) {
