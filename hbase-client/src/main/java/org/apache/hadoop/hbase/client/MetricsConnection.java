...
public class MetricsConnection {
  ...
  protected static final class CallTracker {
    ...
    @VisibleForTesting ConcurrentHashMap<String,Timer> callTimers;
    @VisibleForTesting ConcurrentHashMap<String, Histogram> reqHists;
    @VisibleForTesting ConcurrentHashMap<String, Histogram> respHists;
    ...
    private CallTracker(MetricsRegistry registry, String name, String subName, String scope) {
      ...
      callTimers = new ConcurrentHashMap<String,Timer>();
      reqHists = new ConcurrentHashMap<String,Histogram>();
      respHists = new ConcurrentHashMap<String,Histogram>();
    }
    ...
    public void updateRpc(String tableName, CallStats stats) {
      if(!callTimers.containsKey(tableName)){
        Timer callTimer = registry.newTimer(MetricsConnection.class, DRTN_BASE + tableName + this.name, scope);
        callTimers.putIfAbsent(tableName, callTimer);
      }
      callTimers.get(tableName).update(stats.getCallTimeMs(), TimeUnit.MILLISECONDS);

      if(!reqHists.containsKey(tableName)){
        Histogram reqHist = registry.newHistogram(MetricsConnection.class, REQ_BASE + tableName + this.name, scope);
        reqHists.putIfAbsent(tableName, reqHist);
      }
      reqHists.get(tableName).update(stats.getRequestSizeBytes());

      if(!respHists.containsKey(tableName)){
        Histogram respHist = registry.newHistogram(MetricsConnection.class, RESP_BASE + tableName + this.name, scope);
        respHists.putIfAbsent(tableName, respHist);
      }
      respHists.get(tableName).update(stats.getResponseSizeBytes());
    }
  ...
  public MetricsConnection(final ConnectionManager.HConnectionImplementation conn) {
    ...
    this.metaCacheNumClearServer = registry.newCounter(this.getClass(), "metaCacheNumClearServer", scope);
    this.metaCacheNumClearRegion = registry.newCounter(this.getClass(), "metaCacheNumClearRegion", scope);
  ...
  public void updateRpc(MethodDescriptor method, Message param, CallStats stats) {
    ...
    if (method.getService() == ClientService.getDescriptor()) {
      ...
      case 0:
        ...
        GetRequest getRequest = (GetRequest) param;
        regionName = Bytes.toStringBinary(getRequest.getRegion().getValue().toByteArray());
        tableName = regionName.split(",")[0];
        getTracker.updateRpc(tableName, stats);
        return;
      case 1:
        assert "Mutate".equals(method.getName());
        MutateRequest mutateRequest = (MutateRequest) param;
        final MutationType mutationType = mutateRequest.getMutation().getMutateType();
        regionName = Bytes.toStringBinary(mutateRequest.getRegion().getValue().toByteArray());
        tableName = regionName.split(",")[0];
        switch(mutationType) {
        case APPEND:
          appendTracker.updateRpc(tableName, stats);
          return;
        case DELETE:
          deleteTracker.updateRpc(tableName, stats);
          return;
        case INCREMENT:
          incrementTracker.updateRpc(tableName, stats);
          return;
        case PUT:
          putTracker.updateRpc(tableName, stats);
          return;
        default:
      ...
      case 2:
        ...
        if(scannerId == 0L){
          regionName = Bytes.toStringBinary(scanRequest.getRegion().getValue().toByteArray());
          tableName = regionName.split(",")[0];
        } else{
          // TODO how to obtain tableName through scannerId
        }
        scanTracker.updateRpc(tableName, stats);
        return;
      case 3:
      ...
      case 6:
        ...
        MultiRequest multiRequest = (MultiRequest) param;
        if(multiRequest.getRegionActionCount() > 0){
          regionName = Bytes.toStringBinary(multiRequest.getRegionActionList().get(0).getRegion().getValue().toByteArray());
          tableName = regionName.split(",")[0];
        }
        multiTracker.updateRpc(tableName, stats);
        return;
      default:
