...
public abstract class RegionServerCallable<T, S> implements RetryingCallable<T> {
  ...
  public T call(int callTimeout) throws IOException {
    try {
      ...
      if (getRpcController() != null) {
        ...
        if (shadedRpcController instanceof PayloadCarryingRpcController) {
  ...
  protected CellScanner getRpcControllerCellScanner() {
    return (getRpcController() != null && getRpcController() instanceof PayloadCarryingRpcController)?
        ((PayloadCarryingRpcController)getRpcController()).cellScanner(): null;
  }

  protected void setRpcControllerCellScanner(CellScanner cellScanner) {
    if (getRpcController() != null && getRpcController() instanceof PayloadCarryingRpcController) {
