...
public class ReplicationPeersZKImpl extends ReplicationStateZKBase implements ReplicationPeers {
  ...
  @Override
  public void init(String group) throws ReplicationException {
    try {
  ...
  public void addPeer(String id, ReplicationPeerConfig peerConfig, String tableCFs)
    ...
    try {
      ...
      List<ZKUtilOp> listOfOps = new ArrayList<ZKUtil.ZKUtilOp>(3);
   ...
   */
  private void addExistingPeers(String group) throws ReplicationException {
    ...
    if (znodes != null) {
      for (String z : znodes) {
        // [GROUP]SOURCE_TARGET_index
        if (ReplicationUtil.targetPeerNotBelongTo(z, group)) {
          LOG.info("peerId " + z + " don't belongs to our group[ " + group + "], bypass it.");
          continue;
        }
        createAndAddPeer(z);
      }
